    ...
    def MsgReadThreadFunc(self):
        try:
            while not self._terminated:
                ...
                if can_num:
                    while can_num:
                        read_cnt = 100 if can_num >= 100 else can_num
                        can_msgs, act_num = self._zcan.Receive(self._can_handle, read_cnt, 100)
                        if act_num: 
                            #update data
                ...
                if canfd_num:
                    while canfd_num:
                        read_cnt = 100 if canfd_num >= 100 else canfd_num
                        canfd_msgs, act_num = self._zcan.ReceiveFD(self._can_handle, read_cnt, 100)
                        if act_num: 
                            #update data
                            self._rx_cnt += act_num 
                            self.strvRxCnt.set(str(self._rx_cnt))
